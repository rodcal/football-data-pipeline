    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error("403 Forbidden — check if your token has access to this competition.")
            ...
            logger.warning(f"Connection error on attempt {attempt}. Retrying in {RATE_LIMIT_SLEEP}s...")
            time.sleep(RATE_LIMIT_SLEEP)
 
        except requests.exceptions.Timeout:
            logger.warning(f"Timeout on attempt {attempt}. Retrying in {RATE_LIMIT_SLEEP}s...")
            time.sleep(RATE_LIMIT_SLEEP)
