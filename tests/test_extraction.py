...
class TestFetch:
    def test_successful_fetch(self, sample_matches_response):
        ...
        with (
            patch("import_api.requests.get", return_value=mock_response),
    ...
    def test_retries_on_rate_limit(self):
        ...
        with (
            patch("import_api.requests.get", side_effect=[mock_429, mock_200]),
    ...
    def test_raises_after_max_retries(self):
        ...
        with (
            patch(
                "import_api.requests.get", side_effect=req.exceptions.ConnectionError
            ),
            patch("import_api.time.sleep"),
            patch.dict("os.environ", {"FOOTBALL_API_TOKEN": "test_token"}),
        ):
...
class TestLoadScorers:
    ...
    def test_coalesces_null_goals(self):
        ...
        with patch(
            "load_to_postgres.execute_values", side_effect=capture_execute_values
        ):
            from load_to_postgres import load_scorers
